...
#             print(boxes)
            h, w = image.shape[:-1] 
            #-----------
            # outs: [80x80x(num_classes+4), 40x40x(num_classes+4), 20x20x(num_classes+4)]
            #-----------
            outs = self.compute_targets_each_image(boxes, class_id) 
            images.append(image)
            batch_outs.append(outs)
        
        batch_outs_1 = np.stack([outs[0] for outs in batch_outs])
        batch_outs_2 = np.stack([outs[1] for outs in batch_outs])
        batch_outs_3 = np.stack([outs[2] for outs in batch_outs])
        batch_outs = [batch_outs_1, batch_outs_2, batch_outs_3]
        return np.array(images, dtype=np.float32), batch_outs
    ...
    def compute_targets_per_stride(self, boxes, class_id, stride):
        output_size = self.input_size // stride
        hm = np.zeros((output_size, output_size, self.num_classes), dtype=np.float32)
        whm = np.zeros((output_size, output_size, 2), dtype=np.float32)
        reg = np.zeros((output_size, output_size, 2), dtype=np.float32)
        for i, (box, cls_id) in enumerate(zip(boxes, class_id)):
            #scale box to output size
            xmin, ymin, xmax, ymax = [p/stride for p in box]
...
#         print('------')
    ...
    def compute_targets_each_image(self, boxes, class_id):
        ...
        for stride in self.strides:
            hm, whm, reg = self.compute_targets_per_stride(boxes, class_id, stride)
            out = np.concatenate([whm, reg, hm], -1)
            outs.append(out)
        return outs
