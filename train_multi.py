...
from models.model import MultiHeadModel
import numpy as np
import math
...
model = MultiHeadModel(hparams['backbone'], hparams['neck'], hparams['head'],
...
for epoch in range(resume_epoch, hparams['epochs']+1):
    ...
    for n_batch, d  in pbar:
        step = epoch*n_batch
        #labels: [80x80x(num_classes+4), 40x40x(num_classes+4), 20x20x(num_classes+4)]
        #preds : [80x80x(num_classes+4), 40x40x(num_classes+4), 20x20x(num_classes+4)]
        images, labels = d 
        
        with tf.GradientTape() as tape:
            preds = model(images, training=True)
            
            loss_dict1, total_loss1 = loss(preds[0], labels[0])
            loss_dict2, total_loss2 = loss(preds[1], labels[1])
            loss_dict3, total_loss3 = loss(preds[2], labels[2])
            print(loss_dict1, total_loss1, loss_dict2, total_loss2, loss_dict3, total_loss3)
            loss_dict = {k:loss_dict1[k] + loss_dict2[k] + loss_dict3[k] for k in loss_dict1.keys()}
...
#     if not os.path.exists(anno_json):
    ...
    generate_coco_format_predict_multi(val_gen, model, pred_json, score_threshold=0.01)
